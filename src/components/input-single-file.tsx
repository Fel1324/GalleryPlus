...
export const inputSingleFileVariants = tv({
  base: "flex flex-col justify-center items-center w-full border border-solid border-border-primary group-hover:border-border-active rounded-lg gap-1 transition",
  variants: {
    size: {
      md: "px-5 py-6",
    },
      ...
      {!formFile || !isValidFile() ? (
        <>
          <div className="w-full relative group cursor-pointer">
            <input
              className="opacity-0 cursor-pointer absolute top-0 right-0 w-full h-full"
              type="file"
              {...props}
            />
          ...
          <div className="flex flex-col gap-1 mt-1">
            ...
            {formFile && !isValidSize() && (
              <Text variant="label-small" className="text-accent-red">
                Tamanho do arquivo ultrapassa o limite de {maxFileSizeInMB}MB
              </Text>
            )}
            {error && (
              <Text variant="label-small" className="text-accent-red">
