...
export function InputText({ size, disabled, className, icon, error, ...props}: InputTextProps) {
  return (
    <div className={inputTextContainerVariants({className})}>
      <div className={inputTextWrapperVariants({size, disabled})}>
        {icon && <Icon className={inputTextIconVariants({size})} svg={icon} />}
        <input className={inputTextVariants()} disabled={disabled as boolean} type="text" {...props} />
      </div>
